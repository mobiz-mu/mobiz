...
export type City = {
  ...
  areas: string[];
  /** Types of businesses genuinely common in this area — factual, drives unique local copy */
  localBusinesses: string[];
  /** One factual phrase describing the area's commercial character */
  character: string;
...
export type ServiceTemplate = {
  ...
  /** Genuinely local context — references the area's real business mix + character */
  localContext: (city: City) => string[];
  features: (city: City) => { title: string; description: string }[];
  benefits: (city: City) => string[];
  process: (city: City) => { title: string; description: string }[];
...
export const cities: City[] = [
  {
    ...
    localBusinesses: ["import and export traders", "corporate offices", "retail shops", "financial and professional services", "government-facing suppliers"],
    character: "the capital and main commercial, financial and port hub of Mauritius",
  ...
  },
  {
    ...
    localBusinesses: ["established retailers", "professional practices", "restaurants and cafés", "education and training centres"],
    character: "a long-established highland commercial town",
  ...
  },
  {
    ...
    localBusinesses: ["market traders and retailers", "clothing and fashion shops", "restaurants and cafés", "clinics and professional offices"],
    character: "a busy retail town known for its market and high street",
  ...
  },
  {
    ...
    localBusinesses: ["local shops and services", "trades and contractors", "clinics", "small SMEs"],
    character: "part of the residential Vacoas-Phoenix conurbation",
  ...
  },
  {
    ...
    localBusinesses: ["shops and boutiques", "restaurants", "creative and service businesses", "professional offices"],
    character: "a dense commercial town with strong foot traffic and an arts scene",
  ...
  },
  {
    ...
    localBusinesses: ["restaurants and bars", "hotels and guesthouses", "villa and holiday rentals", "tour and watersports operators", "beauty and wellness spas", "boutiques"],
    character: "the tourism and hospitality capital of the north",
  ...
  },
  {
    ...
    localBusinesses: ["local retailers", "service businesses", "trades", "clinics and practices"],
    character: "a residential and commercial town beside Rose Hill",
  ...
  },
  {
    ...
    localBusinesses: ["retailers and Flacq market traders", "agricultural and food businesses", "east-coast tourism operators", "trades and services"],
    character: "the main commercial town of the eastern region",
  ...
  },
  {
    ...
    localBusinesses: ["retailers and shopping-centre outlets", "light-industrial and warehousing businesses", "distributors", "trades and services"],
    character: "a retail and light-industrial centre",
    nearby: ["vacoas", "quatre-bornes"],
...
export const serviceTemplates: Record<CityServiceKey, ServiceTemplate> = {
  "website-design": {
    ...
    localContext: (c) => [
      `${c.name} is ${c.character}. The businesses here — ${c.localBusinesses.slice(0, 3).join(", ")} and others — compete for the same local customers, and a clear, fast website is one of the cheapest ways to stand out.`,
      `We build sites that suit how ${c.name} customers actually behave: browsing on their phones, checking you're real and reachable, then messaging on WhatsApp. That matters as much in ${c.areas[0]} as it does across the rest of ${c.region}.`,
    ],
    features: (c) => [
      { title: `Designed for ${c.name} businesses`, description: `A premium layout shaped around how ${c.localBusinesses[0]} and similar ${c.name} businesses actually win customers — not a recycled template.` },
      ...
      { title: "SEO-ready structure", description: `Clean headings, fast loading and proper metadata so Google can rank you for searches around ${c.name}.` },
    ...
    benefits: (c) => [
      ...
      `Reach the ${c.localBusinesses[0]} and other buyers active in the area`,
      "Convert more visitors into WhatsApp and phone enquiries",
      `Rank for "near me" searches across ${c.name} and ${c.region}`,
      "Build long-term trust with a professional brand presence",
    ],
    process: (c) => [
      { title: "Discovery", description: `We learn your ${c.name} business, your customers and your goals in a short consultation.` },
  ...
  "seo-services": {
    ...
    localContext: (c) => [
      `${c.name} is ${c.character}, so local search matters here. When someone nearby looks for ${c.localBusinesses[0]} or the services you offer, being on the first page — and on Google Maps — is what decides who gets the call.`,
      `We focus on the searches that actually happen around ${c.name}: "near me" queries, ${c.areas[0]} and ${c.areas[1]} area terms, and the services the ${c.localBusinesses.slice(0, 2).join(" and ")} in your market compete for.`,
    ],
    features: (c) => [
      { title: `${c.name} keyword strategy`, description: `We target the exact terms customers around ${c.name} and ${c.areas[0]} actually search for.` },
      ...
      { title: "Local landing pages", description: `Pages built around your services and the ${c.region} areas you serve, to capture more searches.` },
    ...
    benefits: (c) => [
      ...
      `Win more "near me" searches across ${c.name} and ${c.region}`,
      `Compete with the other ${c.localBusinesses[0]} for local visibility`,
      "Reduce reliance on paid ads over time",
      "Build a compounding, long-term traffic asset",
    ],
    process: (c) => [
      { title: "Audit", description: `We assess your current rankings, site health and local presence in ${c.name}.` },
  ...
  "accounting-services": {
    ...
    localContext: (c) => [
      `${c.name} is ${c.character}, and the ${c.localBusinesses.slice(0, 2).join(" and ")} based here face the same MRA deadlines as everyone else — VAT, PAYE and tax filings don't wait. Falling behind is where penalties and stress creep in.`,
      `We support ${c.name} businesses of different sizes, from a single-owner shop in ${c.areas[0]} to established companies across ${c.region}, scaling the bookkeeping and filing work to what each one actually needs.`,
    ],
    features: (c) => [
      { title: "Bookkeeping", description: `Accurate, up-to-date records for your ${c.name} business so your numbers are always ready.` },
      ...
      { title: "Compliance peace of mind", description: "MRA deadlines tracked so you avoid penalties and surprises." },
    ],
    benefits: (c) => [
      `Stay compliant with Mauritian tax and VAT rules`,
      `Suit ${c.localBusinesses[0]} and other ${c.name} businesses of any size`,
    ...
    process: (c) => [
      { title: "Review", description: `We assess your current records, obligations and deadlines for your ${c.name} business.` },
...
function buildPage(service: ServiceTemplate, city: City): CityServicePageData {
  ...
  return {
    ...
    localContextTitle: `${service.label} for businesses across ${city.name}`,
    localContextParagraphs: service.localContext(city),
    localBusinesses: city.localBusinesses,
    features: service.features(city),
    benefitsTitle: `Why ${city.name} businesses choose MoBiz.mu`,
    benefits: service.benefits(city),
    processTitle: "How we work",
    process: service.process(city),
    faqs: [
      ...service.faqs(city),
      {
        question: `What kind of ${city.name} businesses do you work with?`,
        answer: `We work with a range of ${city.name} businesses, including ${city.localBusinesses.join(", ")}. Whether you're just starting out or already established, we scale the work to your size and goals.`,
      },
    ],
    relatedLinks,
  };
